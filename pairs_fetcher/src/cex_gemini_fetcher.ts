...
export class CEXGeminiFetcherApprox {
    ws: WebSocket;
            ...
            this.ws.on("message", (msg) => {
                ...
                if (data.socket_sequence === curSeqNum) {
                    if (data.events[0].symbol === data.events[1].symbol &&
                        ...
                        newPairs[data.events[0].symbol].rateA = 0.996;
                    ...
                    reject(new Error("Gemini: Out of sequence"));
                }
            });
...
export class CEXGeminiFetcher {
    ...
    private bidOrders: { [price: string]: string };
    private askOrders: { [price: string]: string };
    ...
    updatePairs(pairs): Promise<any> {
        let prom = new Promise((resolve, reject) => {
            ...
            this.ws = new WebSocket(`wss://api.gemini.com/v1/multimarketdata?symbols=${symbols.join()}&bids=true&offers=true&trades=false&auctions=false`);
            this.ws.on("message", (msg) => {
                let data = JSON.parse(msg);
                if (data.socket_sequence === curSeqNum) {
                    let symbol = data.events[0].symbol;
                    console.log(`${symbol}: ${data.events.length}`);
                    for (let i = 0; i < data.events.length; i++) {
                        let event = data.events[i];
                        assert(event.symbol === symbol, `Different symbol! Got ${event.symbol}. Expected ${symbol}.`);
                        if (event.side === "bid") {
                            this.bidOrders[event.price] = event.remaining;
                        } else {
                            this.askOrders[event.price] = event.remaining;
                        }
                    }
